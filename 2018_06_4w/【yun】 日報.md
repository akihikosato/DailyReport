...
#### Android Studio
  1. ImageView
    - urlの写真を2枚まで持ってくるのに成功したが、どうしても一番最後に入れた写真しか出てこない
    - ListViewの中にCustomしたImageViewを入れる形だが、理解不足でListViewにすべての写真を入れようとしたのが問題
    - Custom ImageViewに入れて、複数のイメージを呼び出すことの成功

  2. response.body()
    - retrofit2でtwitterの情報を持ってきたらしいが、応用法が良くわからない
    - response.bodyのなかのlistを他の位置で使おうとしたらNullになってしまい、retrofitについて改めて調べる
...
|15:00 ~| Android Studio | retrofit2 |
|16:00 ~| 報告会 |  |
...
#### 勤怠システム
  1. Excel内容
    - 定期券に関する内容がExcelには入ってない
    - 個人情報ページに情報を入力したらExcelにも記載される

  2. ページング
    - 今月ではなく他の月を検索したままにページングができない
    - クエリストリングで選択した年月も渡してページングするように修正

  3. CSS
    - ボタンの大きさがシフト履歴と違っていたので合わせる

#### Android Studio
  1. ImageView
    - Glideを利用してLinerLayoutにImageViewを生成、urlのイメージを持ってくるのまでは成功した。
    - 複数のイメージの呼び出しと、既に存在するImageViewにイメージを入れるのは明日続いてする予定。
...
|時間  |概要  |詳細  |
|---|---|---|
|10:00 ~| 勤怠システム | Excel内容 |
|11:00 ~| 勤怠システム |  |
|12:00 ~| 勤怠システム | ページング  |
|13:00 ~| 休憩 |  |
|14:00 ~| 勤怠システム |  |
|15:00 ~| 勤怠システム | CSS |
|16:00 ~| Android Studio | ImageView |
|17:00 ~| Android Studio |  |
|18:00 ~| 日報 |  |
...
|時間  |概要  |詳細  |
|---|---|---|
|10:00 ~| Android Studio | 複数のイメージの出力 |
|11:00 ~| Android Studio |  |
|12:00 ~| Android Studio |  |
|13:00 ~| 休憩 |  |
|14:00 ~| Android Studio |  |
