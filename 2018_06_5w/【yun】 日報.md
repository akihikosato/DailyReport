...
#### 勤怠システム
  1. 管理者画面
    - 念の為、今後の修正がある場合困らないようにコメントをつける。

#### 報告会・準備
  1. 資料作成
    - 今週の作業を顧みながら資料を作成
  2. Android Studio
    - 報告会で見せるようにきれいに調整してビルドしておく   
    - 目標にしていたところまでは作り終わった

#### ファイル整理
  1. 今まで書いていたコードやファイルを整理する

#### 本日のスケジュール

|時間  |概要  |詳細  |
|---|---|---|
|10:00 ~| 勤怠システム | コメントをつける |
|11:00 ~| Android Studio | 発表用の調整 |
|12:00 ~| 資料作成 |  |
|13:00 ~| 休憩 |  |
|14:00 ~| 資料作成 |  |
|15:00 ~| 報告会 |  |
...
#### Android Studio
  1. 全体的な調整
    - アクションバーにあったSearchViewをLieanerLayoutの中に移る
    - 昨日できなかったレイアウトの調整を行う
    - 一枚のイメージを見る場合、上のアクションバーで作成者の名前が見えるようにする。IDはLong形になっていて、元のIDの取り方がわからないので名前を入れた。
  2. 追加機能の工夫
    - この後個人的にでも作業するなら、どんな機能を追加して拡張するか考えた。
      - 期限を選んで検索
      - 利用者のIDで検索
      - スライドで検索したイメージをみれる
